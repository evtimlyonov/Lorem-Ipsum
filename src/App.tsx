...
function App() {
  ...
  const submitHandler: React.FormEventHandler<HTMLFormElement> = (e) => {
    ...
    // console.log(paragraph.current?.value);
  };
  return (
    <main>
      <section className='mx-auto mt-20 w-[90vw] max-w-2xl flex flex-col items-center justify-center'>
        <h1 className='text-3xl font-bold uppercase text-[#063251] tracking-wider mb-8 text-center'>
          tired of boring lorem ipsum?
        </h1>
        <form
          onSubmit={submitHandler}
          className='flex flex-wrap items-center justify-center mb-16 text-xl tracking-wider'>
          ...
          />
          <button className='uppercase bg-[#49a6e9] py-[0.375rem] px-[0.75rem] text-sm rounded tracking-wider'>
            generate
          </button>
        </form>
        {paragraphs.map((p, i) => (
          <p key={i} className='text-[#617d98] tracking-wide mb-5 text-center'>
            {p}
          </p>
        ))}
      </section>
